	...
	public Optional<Financeiro> findById(int id) {
		...
		try (Connection conn = dataSource.getConnection(); PreparedStatement stmt = conn.prepareStatement(sql)) {
			...
			try (ResultSet rs = stmt.executeQuery()) {
				if (rs.next()) {
					Financeiro financeiro = new Financeiro(rs.getInt("id_financeiro"), rs.getFloat("historico_lucro"),
							rs.getFloat("historico_prejuizo"), rs.getDate("data_atualizacao").toLocalDate());
